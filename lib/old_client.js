...
var GiantSwarm = (function () {
  ...
  return function(){
    ...
    var waitForMaxRetries = 10;

    this.setWaitForTimeout = function(miliseconds) {
      waitForTimeout = miliseconds;
    }

    this.setWaitForMaxRetries = function(maxRetries) {
      waitForMaxRetries = maxRetries;
    }
    ...
    this.startService = function(organizationName, environmentName, serviceName, successCallback, errorCallback) {
      ...
      var uri = '/v2/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/service/' + encodeURIComponent(serviceName) + '/start';
    ...
    this.stopService = function(organizationName, environmentName, serviceName, successCallback, errorCallback) {
      ...
      var uri = '/v2/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/service/' + encodeURIComponent(serviceName) + '/stop';
    ...
    this.waitFor = function(organizationName, environmentName, taskId, successCallback, errorCallback) {
      this._validateStringParameters({
        organizationName: organizationName,
        environmentName: environmentName,
        taskId: taskId
      ...
      var uri = '/v2/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/task/' + encodeURIComponent(taskId) + '/waitfor';

      var maxRetries = waitForMaxRetries;
      var retries = 0;
      (function request() {
        retries += 1;
        if (retries === maxRetries) {
          errorCallback("Maximum number of " + retries +  " retries reached");
        } else {
          var result = requestHelper.get({
            url: apiEndpoint + uri,
            onSuccess: successCallback,
            onError: request.bind(this),
            headers: this._headers(),
      ...
      // var response = request.bind(this)();
      // while(! response.ok) {
      //   console.log("======================================")
      //   console.log(response);
      //   console.log("======================================")
      //   response = request.bind(this)();
      // }
    };
   ...
   /**
    ...
    this.startComponent = function(organizationName, environmentName, serviceName, componentName, successCallback, errorCallback) {
      ...
      var uri = '/v2/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName);
