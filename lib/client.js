...
var GiantSwarm = (function () {
  ...
  return {
    ...
    /**
     * Set the object that is used to make API calls
     *
     * @param {object} [agent] The object to be used to make requests. Should
     * be a superagent
     */

    setRequestAgent: function(agent) {
      request = agent;
    },
    ...
    _simple_data_get_request: function(uri, params, successCallback, errorCallback) {
      ...
      request.get(apiEndpoint + uri)
        ...
        .set(headers)
        .end(function(err, resp){
          if (resp && resp.unauthorized && unauthorizedCallback) unauthorizedCallback(resp, err);
     ...
     * @param {Function} [errorCallback] Optional callback to be called in case of an error, error object as argument
     */
    _simple_data_post_request: function(uri, successCallback, errorCallback, params) {
      this._validateStringParameters({uri: uri});
      if (!successCallback) throw new this.UserException("Parameter successCallback must be given");
      if (typeof successCallback !== 'function') throw new this.UserException("Parameter successCallback must be of type function");
      if (errorCallback !== null && typeof errorCallback !== 'function') throw new this.UserException('errorCallback must be null or function');
    ...
    serviceStatus: function(organizationName, environmentName, serviceName, successCallback, errorCallback) {
      ...
      var uri = '/v1/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/service/' + encodeURIComponent(serviceName) + '/status';
      this._simple_data_get_request(uri, null, successCallback, errorCallback);
    },
    ...
    serviceDefinition: function(organizationName, environmentName, serviceName, successCallback, errorCallback) {
      ...
      var uri = '/v1/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/service/' + encodeURIComponent(serviceName) + '/config';
      this._simple_data_get_request(uri, null, successCallback, errorCallback);
    },
   ...
   /**
    ...
    componentStatus: function(organizationName, environmentName, serviceName, componentName, successCallback, errorCallback) {
      ...
      params = JSON.stringify({component: componentName})
      ...
      this._simple_data_post_request(uri, successCallback, errorCallback, params);
    },
