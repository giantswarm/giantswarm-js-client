...
var GiantSwarm = (function () {
  ...
  return {
    ...
    // create object for authenticated request
    _authorizedRequest: function() {
      if (!authToken) throw new UserException('Not authenticated');
      return request.set('Authorization', 'giantswarm ' + authToken);
    },
    ...
    _simple_data_get_request: function(uri, successCallback, errorCallback) {
      ...
      if (typeof(uri) !== 'string') throw new this.UserException("Parameter 'uri' must be String");
      if (uri === '') throw new this.UserException("Parameter 'uri' must not be an empty String");
