...
var GiantSwarm = (function () {
  ...
  return {
     ...
     * @param {Function} [errorCallback] Optional callback to be called in case of an error, error object as argument
     */
    serviceStatus: function(organizationName, environmentName, applicationName, successCallback, errorCallback) {
      this._validateStringParameters({
        organizationName: organizationName,
        environmentName: environmentName,
        applicationName: applicationName
      });
      var uri = '/v1/org/' + encodeURIComponent(organizationName) + '/env/' + encodeURIComponent(environmentName) + '/service/' + encodeURIComponent(applicationName) + '/status';
      this._simple_data_get_request(uri, null, successCallback, errorCallback);
    },
