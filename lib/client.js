...
var GiantSwarm = (function () {
  ...
  return {
    ...
    // */
    //_openWebSocket: function(uri, postPayload, messageCallback, createCallback, errorCallback) {
    //  this._validateStringParameters({
     ...
     */
    streamStats: function(organizationName, instanceIds, messageCallback, createCallback, errorCallback) {
      this._validateStringParameters({
        organizationName: organizationName,
      });
      if (typeof(instanceIds) !== 'object') throw new this.UserException("Parameter 'instanceIds' must be an array");
      ...
      request
        .post(apiEndpoint + uri)
        .set('Authorization', 'giantswarm ' + authToken)
        .send({instances: instanceIds, interval: 5})
        .end(function(err, resp){
          if (err) {
            errorCallback(err);
          } else {
